...
export class ProductsService {
  ...
  constructor(
    ...
    private trendingProductRepository: Repository<TrendingProduct>
  ) {}
  ...
  // TrendingProduct CRUD
  async createTrendingProduct(productId: string, customImage?: string) {
    const product = await this.productsRepository.findOne({
      where: { id: productId },
    });
    if (!product) throw new Error("Product not found");
    const trending = this.trendingProductRepository.create({
      product,
      customImage,
    });
    return this.trendingProductRepository.save(trending);
  }

  async getTrendingProducts() {
    return this.trendingProductRepository.find({
      order: { createdAt: "DESC" },
    });
  }

  async updateTrendingProduct(id: string, customImage?: string) {
    const trending = await this.trendingProductRepository.findOne({
      where: { id },
    });
    if (!trending) throw new Error("Trending product not found");
    trending.customImage = customImage;
    return this.trendingProductRepository.save(trending);
  }

  async deleteTrendingProduct(id: string) {
    return this.trendingProductRepository.delete(id);
  }
}
